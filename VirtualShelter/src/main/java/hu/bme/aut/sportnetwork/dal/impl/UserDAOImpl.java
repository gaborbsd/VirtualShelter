...
public class UserDAOImpl implements UserDAOCustom {
	...
	public List<User> getFriendsOfUser(String name) {
		TypedQuery<FriendShip> query = em.createQuery("SELECT f FROM User u JOIN u.friends f WHERE name = :name", FriendShip.class);
		query.setParameter("name", name);
		List<User> ret = new ArrayList<>();
		List<FriendShip> res = query.getResultList();
		res.forEach(f -> ret.add(f.getFriend()));
		ret.forEach(u -> u.setFriendStatus(FriendStatus.FRIEND));
		return ret;
	}
