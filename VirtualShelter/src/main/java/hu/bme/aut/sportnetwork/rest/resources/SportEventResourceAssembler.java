...
public class SportEventResourceAssembler extends
	...
	protected SportEventResource instantiateResource(SportEvent entity) {
		 ...
		 * resource.setLevel(entity.getLevelIntervalFrom() ==
		 ...
		 * entity.getComments().forEach(c -> WrapperUtils.addComment(comments,
		 * c)); resource.setComments(comments);
		 * 
		 ...
		 * 
		 * 
		 * switch (entity.getStatus()) { case OWNER: resource.setStatus(0);
